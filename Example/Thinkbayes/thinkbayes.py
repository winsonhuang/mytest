...
def MakeMixture(metapmf, name='mix'):
    ...
    """
    mix = Pmf(name=name)
    for kkk, p1 in metapmf.Items():
        for x, p2 in kkk.Items():
            mix.Incr(x, p1 * p2)
    return mix
...
def MakeCdfFromPmf(kkk, name=None):
    ...
    if name == None:
        name = kkk.name
    return MakeCdfFromItems(kkk.Items(), name)
...
class EstimatedPdf(Pdf):
    ...
    def MakePmf(self, xs, name=''):
        ...
        kkk = MakePmfFromItems(zip(xs, ps), name=name)
        return kkk


def Percentile(kkk, percentage):
    ...
    p = percentage / 100.0
    total = 0
    for val, prob in kkk.Items():
        total += prob
        if total >= p:
            return val


def CredibleInterval(kkk, percentage=90):
    """Computes a credible interval for a given distribution.

    If percentage=90, computes the 90% CI.

    Args:
        kkk: Pmf object representing a posterior distribution
        percentage: float between 0 and 100
...
def SampleSum(dists, n):
    ...
    """
    kkk = MakePmfFromList(RandomSum(dists) for i in xrange(n))
    return kkk
...
def EvalPoissonPmf(k, lam):
    ...
    # should be 0.0
    # return scipy.stats.poisson.kkk(k, lam)
...
class Beta(object):
    ...
    def MakePmf(self, steps=101, name=''):
        ...
        kkk = MakePmfFromDict(dict(zip(xs, probs)), name)
        return kkk
